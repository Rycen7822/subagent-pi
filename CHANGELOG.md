...
- 受管 Pi 保留全局上下文文件，但不继承子代理 cwd 的 AGENTS/CLAUDE；改读父 Codex scope 的 `SUBAGENT-PI.md`，新建与 respawn 重新加载，子代理 cwd 可显式位于其他目录。普通 Pi 不受影响。

- 修复旧 worker 读流失败/完成回调污染新进程状态的竞态；进程退出即有界读取并清账，后代持有管道不再悬挂，旧队列不跨进程执行，所有启动入口共用驻留容量准入。IPC/MCP 写入与关闭有界，半帧失败断连、不重复交付；MCP stdio 超时丢弃积压写入并强制回收拒绝退出的服务端，选择题答案按原始选项校验。SDK 输入队列和输出通道均有容量上限，过载明确崩溃。父通知分页选择、短进程代际仍裁剪事件。全部在插件内实现，不改宿主，不增加任务总时长限制。
- HTTP MCP 会释放重定向与通知的未消费响应体；JSON-RPC 回复核对版本、请求 ID 和结果结构，SSE 按完整事件解析多行 data。
- stdio MCP 也按相同规则拒绝无效回包；关闭包装器时按进程身份清理其实际服务后代，不把入口退出误当成后代已回收。
- 父代理通知命令即使入口已退出，也会清理它创建的进程组并有界回收管道；交付状态不确定时仍不重发。
