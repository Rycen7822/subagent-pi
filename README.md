...
这是完整源码版 `0.2.9`，针对 **Linux / WSL2，Python 3.11+**。运行时使用 Python 标准库、Node.js 22.19+ 和已安装 Pi 的 SDK，无额外 pip/npm 构建依赖；Pi 和 Codex 需要你已自行安装。它不是 Codex 内建 collaboration runtime，不进入原生 `/agents`，也不承诺无 hooks 的自动唤醒。
