...
a=p.parse_args()
files=[f for f in sorted(ROOT.rglob('*')) if f.is_file() and not any(part in {'__pycache__','.git','.pytest_cache','.venv'} for part in f.parts)
       and f.suffix not in {'.pyc','.zip'} and f.name!='FILES.sha256']
